...
def register(db : db_dependecy, user_register_model : User_register_model ):
    ...
    new_user = User(username=user_register_model.username, hashed_password=hashed)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
# if you are here jorge, this is a test about protected route, ok ?
@app.get("/protected")
def protegido(usuario: User = Depends(get_current_user)):
    return {"msg": f" {usuario.username}, authenticated uhuhuhuhu"}
